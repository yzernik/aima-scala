 ...
 * function of time.
 *
 * <b>Note:</b> Supports both Tree and Graph based versions by using GraphNodeExpander.nodeExpander or
...
object simulatedAnnealing {
  ...
  def apply[S, A](nodeExpander: NodeExpander[S, A])
    ...
    @tailrec
    def recur(
        nodeExpander: NodeExpander[S, A],
        times: Stream[Int],
        current: Node[S, A],
        shuffled: Traversable[Node[S, A]]): SearchResult[(S, Seq[A])] = {
      ...
      shuffled.headOption match {
        case Some(neighbor) =>
          ...
          val next = if (deltaE > 0 || Random.nextDouble() <= exp(deltaE / T)) neighbor else current
          val (children, newNodeExpander) = nodeExpander(problem, next)
          recur(newNodeExpander, times.tail, next, shuffle(children))
        case None => Failure
      }
    }
    val startNode = Node[S, A](problem.initialState, None, None, 0)
    val (children, newNodeExpander) = nodeExpander(problem, startNode)
    recur(newNodeExpander, Stream.from(0), startNode, shuffle(children))
  }
}
